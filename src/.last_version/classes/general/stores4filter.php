...
class CIMYIEStores4Filter
{
	function OnStoreProductUpdateHandler($id,$arFields)
	{
		...
		{
			$imyie_s4f_iblocks = unserialize( COption::GetOptionString("imyie.stores4filter", "imyie_s4f_iblocks", array() ) );
			$IBLOCK_ID = $arElement["IBLOCK_ID"];
			if(CModule::IncludeModule('iblock') && CModule::IncludeModule('catalog') && in_array($IBLOCK_ID,$imyie_s4f_iblocks) && $PROPERTY_CODE!="")
			{
	...
	function CheckProperty($IBLOCK_ID,$PROPERTY_CODE)
	{
		// stores
		$arrValues = array();
		...
		$dbProperties = CIBlockProperty::GetList(array("sort"=>"asc","name"=>"asc"),array("IBLOCK_ID"=>$IBLOCK_ID,"CODE"=>$PROPERTY_CODE));
